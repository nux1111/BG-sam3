...
class SAM3ModelWrapper:
    ...
    def model_size(self):
        ...
        if self._model_size is None:
            # Calculate size manually to handle quantized models
            # ComfyUI's module_size breaks on quantized parameters
            try:
                total_size = 0
                for param in self.model.parameters():
                    if hasattr(param, 'nbytes'):
                        ...
                        # Fallback for quantized tensors - estimate 1 byte per element
                        total_size += param.numel()
                for buffer in self.model.buffers():
                    if hasattr(buffer, 'nbytes'):
                        total_size += buffer.nbytes
                    elif hasattr(buffer, 'numel') and hasattr(buffer, 'element_size'):
                        total_size += buffer.numel() * buffer.element_size()
                self._model_size = total_size
            except Exception:
                # Ultimate fallback - estimate based on parameter count
                param_count = sum(p.numel() for p in self.model.parameters())
                self._model_size = param_count * 2  # Assume FP16
