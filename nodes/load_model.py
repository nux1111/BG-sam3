...
class SAM3UnifiedModel(SAM3ModelPatcher):
    ...
    def unpatch_model(self, device_to=None, unpatch_weights=True):
        ...
        self._video_predictor.model.to(device_to)
        # Clear caches to free VRAM (position encoding cache, attention freqs_cis, etc.)
        self.clear_caches(self._video_predictor.model)
        gc.collect()
        if torch.cuda.is_available():
            torch.cuda.empty_cache()

    def clear_caches(self, model):
        """Recursively clear dictionary caches and attention tensors from all submodules."""
        if model is None:
            return
        # Call clear_cache on this module if it has the method
...
class LoadSAM3Model:
    ...
    Precision options:
    - FP16: Half precision (~3.5GB VRAM) - recommended, minimal quality loss
    - FP32: Full precision (~7GB VRAM) - best quality, more VRAM

    Note: INT8 quantization is not available because PyTorch's dynamic
    quantization only supports CPU inference, which would be too slow.
    """
        ...
        return {
            "required": {
                ...
                "precision": (cls.PRECISION_OPTIONS, {
                    "default": "fp16",
                    "tooltip": "Model precision: fp16 (recommended, ~3.5GB), fp32 (full, ~7GB)"
                }),
            },
        }
    ...
    def load_model(self, model_path, precision="fp16"):
        """
        ...
        # Apply precision conversion based on user selection
        model_to_convert = video_predictor.model
        original_size = sum(p.numel() * p.element_size() for p in model_to_convert.parameters()) / 1024 / 1024
        print(f"[SAM3] Original model size: {original_size:.1f} MB (before precision conversion)")

        if precision == "fp16":
            print(f"[SAM3] Converting to FP16 (half precision)...")
            model_to_convert.half()
            # Also convert any float32 buffers
            for buffer_name, buffer in model_to_convert.named_buffers():
                if buffer.dtype == torch.float32:
                    try:
                        buffer.data = buffer.data.half()
                    except Exception:
                        pass  # Some buffers can't be converted (e.g., indices)
            print(f"[SAM3] Converted to FP16")
        # FP32 is the default, no conversion needed

        # Calculate new size after conversion
        new_size = sum(p.numel() * p.element_size() for p in video_predictor.model.parameters()) / 1024 / 1024
        print(f"[SAM3] Model size after {precision}: {new_size:.1f} MB")
        ...
        print(f"[SAM3] Unified model ready (precision: {precision}, size: {unified_model.model_size() / 1024 / 1024:.1f} MB)")
