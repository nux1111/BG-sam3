...
def _get_autocast_dtype():
    """Get the appropriate autocast dtype based on GPU capability."""
    if not torch.cuda.is_available():
        return None  # No autocast on CPU
...
class Sam3Processor:
    ...
    def _forward_grounding(self, state: Dict):
        # Note: We don't use autocast here because:
        # 1. The model weights are already FP16 (from LoadSAM3Model precision setting)
        # 2. Cached backbone_out tensors from previous runs are also FP16
        # 3. autocast conflicts with grid_sample when tensors are already FP16
        # Instead, dtype consistency is handled at the layer level (geometry_encoders, decoder, encoder)
