...
class SequenceGeometryEncoder(nn.Module):
    ...
    def _encode_points(self, points, points_mask, points_labels, img_feats):
        ...
        if self.points_direct_project is not None:
            # Cast points to match layer dtype (fixes Float/Half mismatch in FP16 mode)
            points_casted = points.to(self.points_direct_project.weight.dtype)
            proj = self.points_direct_project(points_casted)
        ...
        if self.points_pool_project is not None:
            ...
            # re normalize to [-1, 1]
            grid = (grid * 2) - 1
            # grid_sample requires float32 on some PyTorch versions
            sampled = torch.nn.functional.grid_sample(
                img_feats.float(), grid.float(), align_corners=False
            )
            assert list(sampled.shape) == [bs, self.d_model, n_points, 1]
            sampled = sampled.squeeze(-1).permute(2, 0, 1)
            # Cast to match layer dtype (fixes Float/Half mismatch in FP16 mode)
    ...
    def _encode_boxes(self, boxes, boxes_mask, boxes_labels, img_feats):
        ...
        if self.boxes_direct_project is not None:
            # Cast boxes to match layer dtype (fixes Float/Half mismatch in FP16 mode)
            boxes_casted = boxes.to(self.boxes_direct_project.weight.dtype)
            proj = self.boxes_direct_project(boxes_casted)
        ...
        if self.boxes_pool_project is not None:
            ...
            # Cast boxes to match img_feats dtype (fixes Half/Float mismatch in FP16 mode)
