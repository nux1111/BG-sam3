...
class MultiheadAttentionWrapper(nn.MultiheadAttention):
    def forward(self, query, key, value, *args, **kwargs):
        # Cast inputs to match layer dtype (fixes Float/Half mismatch in FP16 mode)
        target_dtype = self.in_proj_weight.dtype
...
class DotProductScoring(torch.nn.Module):
    ...
    def forward(self, hs, prompt, prompt_mask):
        ...
        # Cast inputs to match layer dtype (fixes Float/Half mismatch in FP16 mode)
        target_dtype = self.prompt_proj.weight.dtype
        ...
        pooled_prompt = pooled_prompt.to(target_dtype)
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        # Cast input to match layer dtype (fixes Float/Half mismatch in FP16 mode)
