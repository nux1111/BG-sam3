...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_ffn(self, tgt):
        with torch.amp.autocast(device_type="cuda", enabled=False):
            # Cast tgt to match linear layer dtype (fixes Float/Half mismatch in FP16 mode)
            tgt_casted = tgt.to(self.linear1.weight.dtype)
            tgt2 = self.linear2(self.dropout3(self.activation(self.linear1(tgt_casted))))
    ...
    def forward(
        ...
        **kwargs,  # additional kwargs for compatibility
        ...
        Input:
            ...
            -
        """
        # Cast inputs to match model dtype (fixes Float/Half mismatch in FP16 mode)
        target_dtype = self.linear1.weight.dtype
        if tgt is not None:
            tgt = tgt.to(target_dtype)
        if tgt_query_pos is not None:
            tgt_query_pos = tgt_query_pos.to(target_dtype)
        if memory is not None:
            memory = memory.to(target_dtype)
        if memory_pos is not None:
            memory_pos = memory_pos.to(target_dtype)
        if memory_text is not None:
            memory_text = memory_text.to(target_dtype)
        if presence_token is not None:
            presence_token = presence_token.to(target_dtype)
        # self attention
...
class TransformerEncoderCrossAttention(nn.Module):
    ...
    def clear_cache(self):
        """Move attention freqs_cis tensors to CPU to free VRAM during offload."""
        for layer in self.layers:
            if hasattr(layer, 'self_attn') and hasattr(layer.self_attn, 'freqs_cis'):
                layer.self_attn.freqs_cis = layer.self_attn.freqs_cis.cpu()
            if hasattr(layer, 'cross_attn_image') and layer.cross_attn_image is not None:
                if hasattr(layer.cross_attn_image, 'freqs_cis'):
                    layer.cross_attn_image.freqs_cis = layer.cross_attn_image.freqs_cis.cpu()
    ...
    def forward_post(
        ...
        **kwargs,
        ...
        if query_pos is not None:
            query_pos = query_pos.to(target_dtype)
        if pos is not None:
